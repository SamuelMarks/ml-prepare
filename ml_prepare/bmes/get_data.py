...
package_root = path.dirname(resource_filename("ml_prepare", "__init__.py"))
cache = Cache(
    fname=environ.get("CACHE_FNAME") if "NO_REDIS" in environ else redis_cursor
)
rand_cache_obj = Cache(
    fname=path.join(package_root, "_data", ".cache", "rand_cache.pkl")
)
rand_cache_recreate = environ.get(
    "RECREATE_RAND_CACHE",
    all(
        (
            isinstance(rand_cache_obj.fname, string_types),
            not path.getsize(rand_cache_obj.fname),
        )
    ),
)
...
def _update_generated_types_py(args=None, replace=False):
    global package_root
    gen_py = path.join(package_root, "bmes", "generated_types.py")
    ...
    with open(gen_py, "wt") as f:
        ...
        if args is None:
            ...
            f.write(
                "T0 = namedtuple({T0_args[0]!r}, {T0_args[1]})\n".format(T0_args=args)
            )
        f.write("\n\ndel namedtuple\n\n__all__ = ['T0']\n")
...
def _get_tbl(xlsx="glaucoma_20161205plus_Age23.xlsx"):
    ...
    tbl = dict(
        imap(
            lambda row: (
                row[0].value,
                RecImg(globals()["generated_types"].T0(*get_vals(row)), None),
            ),
            rows_gen,
        )
    )
    ...
    with SAS7BDAT(sas7bdat, skip_header=True) as f:
        sas_tbl = dict(
            imap(
                lambda row: (
                    row[0],
                    RecImg(globals()["generated_types"].T0(*row), None),
                ),
                f.readlines(),
            )
        )
...
def _populate_imgs(img_directory, skip_save=True):
    ...
    if "id2ideyefname" not in pickled_cache or not len(pickled_cache["id2ideyefname"]):
    ...
    if "imgs_to_id" not in pickled_cache or not len(pickled_cache["imgs_to_id"]):
    ...
    if "total_imgs" not in pickled_cache or not len(pickled_cache["total_imgs"]):
    ...
    logger.debug(
        "# not allocated in tbl:".ljust(just)
...
def _vanilla_stats(skip_save=True):
    ...
    if "oags1" not in pickled_cache or not len(pickled_cache["oags1"]):
        pickled_cache["oags1"] = oags1 = tuple(
            v.rec.IDNUM for v in itervalues(tbl) if v.rec.oag1
        )
    else:
        oags1 = pickled_cache[
            "oags1"
        ]  # Weird, this doesn't get into locals() from `update_locals`

    if "loags1" not in pickled_cache or not len(pickled_cache["loags1"]):
        pickled_cache["loag1"] = loag1 = tuple(
            v.rec.IDNUM for v in itervalues(tbl) if v.rec.loag1
        )
        pickled_cache["roag1"] = roag1 = tuple(
            v.rec.IDNUM for v in itervalues(tbl) if v.rec.roag1
        )
    else:
    ...
    if "no_oags1" not in pickled_cache or not len(pickled_cache["no_oags1"]):
        pickled_cache["no_oags1"] = no_oags1 = tuple(
            v.rec.IDNUM for v in itervalues(tbl) if not v.rec.oag1
        )
    ...
    if "_vanilla_stats" not in pickled_cache or not len(
        ...
        pickled_cache["_vanilla_stats"] = vanilla_stats = "\n".join(
            ...
            for t in (
                ...
                ("# len(sas_tbl) == len(tbl):".ljust(just), len(sas_tbl) == len(tbl)),
            )
        )
        skip_save or cache.save(pickled_cache)
    ...
    if "loags_id2fname" not in pickled_cache or not len(
        ...
        id2fname = lambda dataset, eye: (lambda l: dict(izip(l[::2], l[1::2])))(
            tuple(
                chain.from_iterable(
                    imap(
                        lambda idnum_group: (
                                ...
                                imap(
                                    ...
                                    imap(
                                        lambda ideyefnames: ifilter(
                                            lambda ideyefname: ideyefname.eye == eye,
                                            ideyefnames,
                                        ),
                                        imap(
                                            lambda idnum: id2ideyefname[idnum], dataset
                                        ),
                                    ),
                                )
                            ),
    ...
    logger.debug(
        "generated_types.T0._fields:".ljust(just)
...
def make_symlinks(dest_dir, filenames, clean_dir=False):
    ...
    it_consumes(
        imap(
            lambda fname: symlink(fname, path.join(dest_dir, path.basename(fname))),
            filenames,
        )
    )
...
def get_datasets(no_oags=970, oags=30, skip_save=True):
    ...
    train, test = (
        frozenset(
            chain(
                (
                    no_oags1[k]
                    for k in rand_cache["0-13661"][i * no_oags : no_oags + i * no_oags]
                ),
    ...
    pickled_cache["datasets"] = Datasets(
        train=train, test=test, validation=(train | test) ^ tbl_ids
    )
...
def _log_set_stats():
    ...
    logger.debug(
        "# shared between sets:".ljust(just)
        + str(
            sum(
                (
                    len(datasets.validation & datasets.test),
    ...
    logger.debug(
        "# len(all sets):".ljust(just)
        + str(sum(imap(len, (datasets.train, datasets.test, datasets.validation))))
    )
    logger.debug("# len(total):".ljust(just) + str(len(tbl)))
    logger.debug(
        "# len(all sets) == len(total):".ljust(just)
        + str(
            sum(imap(len, (datasets.train, datasets.test, datasets.validation)))
            == len(tbl)
        )
    )
...
def get_feature_names():
    return tuple(
        field for field in generated_types.T0._fields if field.endswith("1")
    )  # BMES1


def get_features(feature_names):
    return np.bmat(
        np.fromiter((idx for idx, _ in enumerate(feature_names)), np.float32)
    )
...
Data = namedtuple(
    "Data", ("tbl", "datasets", "features", "feature_names", "pickled_cache")
)


def link_distribute_dataset(
    train_ratio,
    test_ratio,
    ...
    def n_stats(dataset):
        _no_glaucoma_n = int(
            outer_locals["{}_ratio".format(dataset)] * len_no_glaucoma_fnames
        )
        _glaucoma_n = int(
            outer_locals["{}_positive_ratio".format(dataset)] * len_glaucoma_fnames
        )
        logger.debug(
            "# of negative {} images:".format(dataset).ljust(just)
            + "{:d}".format(_no_glaucoma_n)
        )
        logger.debug(
            "# of positive {} images:".format(dataset).ljust(just)
            + "{:d}".format(_glaucoma_n)
        )
    ...
    train_glaucoma_fnames = idx_to_tuple(
        glaucoma_fnames, rand_glaucoma[:train_glaucoma_n]
    )
    train_no_glaucoma_fnames = idx_to_tuple(
        bmes1_no_glaucoma_fnames, rand_no_glaucoma[:train_no_glaucoma_n]
    )
    if max_imgs:
    ...
    make_symlinks(
        path.join(train_dir, "glaucoma"),
        filenames=train_glaucoma_fnames,
        clean_dir=True,
    )
    make_symlinks(
        path.join(train_dir, "no_glaucoma"),
        filenames=train_no_glaucoma_fnames,
        clean_dir=True,
    )

    test_glaucoma_fnames = idx_to_tuple(
        glaucoma_fnames,
    ...
    make_symlinks(
        path.join(test_dir, "glaucoma"), filenames=test_glaucoma_fnames, clean_dir=True
    )
    make_symlinks(
        path.join(test_dir, "no_glaucoma"),
        filenames=test_no_glaucoma_fnames,
        clean_dir=True,
    )

    valid_glaucoma_fnames = idx_to_tuple(
        glaucoma_fnames, rand_glaucoma[train_glaucoma_n + test_glaucoma_n :]
    )
    valid_no_glaucoma_fnames = idx_to_tuple(
        bmes1_no_glaucoma_fnames,
    ...
    make_symlinks(
        path.join(valid_dir, "glaucoma"),
        filenames=valid_glaucoma_fnames,
        clean_dir=True,
    )
    make_symlinks(
        path.join(valid_dir, "no_glaucoma"),
        filenames=valid_no_glaucoma_fnames,
        clean_dir=True,
    )
...
def get_data(
    ...
    :rtype: ``Datasets``
    """
    global pickled_cache, cache, package_root

    part = "BMES123"
    if not path.isdir(path.join(base_dir, part)):
        if path.basename(base_dir) == part:
            base_dir = path.dirname(base_dir)
        else:
            raise IOError(
                "Wrong base_dir provided, "
                "need one with {!r} inside, got: {!r}".format(part, base_dir)
            )
    ...
    if invalidate:
        cache.invalidate()
    pickled_cache = (
        cache.update_locals(cache.load(), locals())
        if path.getsize(path.join(package_root, "bmes", "generated_types.py")) > 50
        else {}
    )

    if pickled_cache:
        logger.debug(
            "imported T0 has:".ljust(just)
            + "{}".format(
                generated_types.T0._fields
                if generated_types.T0 is not None
                else generated_types.T0
            )
        )
    ...
    if path.basename(base_dir) == "BMES123":
        base_dir = path.dirname(base_dir)
    _populate_imgs(img_directory=path.join(base_dir, "BMES123"), skip_save=skip_save)
    _vanilla_stats(skip_save=skip_save)

    if "glaucoma_fnames" not in pickled_cache or not len(
        pickled_cache["glaucoma_fnames"]
    ):
        all_imgs = pickled_cache["all_imgs"]
        loags_id2fname = pickled_cache["loags_id2fname"]
        roags_id2fname = pickled_cache["roags_id2fname"]
        pickled_cache["glaucoma_fnames"] = glaucoma_fnames = frozenset(
            chain.from_iterable(
                chain.from_iterable(
                    (itervalues(loags_id2fname), itervalues(roags_id2fname))
                )
            )
        )
    ...
    if rand_cache_recreate:
        rand_cache = {}
        len_glaucoma_fnames = len(glaucoma_fnames)
        assert len_glaucoma_fnames > 0
        len_no_glaucoma_fnames = len(bmes1_no_glaucoma_fnames)
        assert len_no_glaucoma_fnames > 0
    ...
    return link_distribute_dataset(
...
def prepare_bmes_splits(root_dir):  # type: (str) -> Iterator[SplitFolder]
    ...
    def process_split(split):
        ...
        #   - ...
        num_examples = sum(
            len(listdir(path.join(folder, dirname))) for dirname in listdir(folder)
        )
        if num_examples == 0:
...
from ml_prepare.bmes import generated_types
...
# data_dir="$HOME"'/OneDrive - The University of Sydney (Students)/BMES123' generate="$(mktemp -d)" tfds="$(mktemp -d)"; python -m ml_prepare --dataset bmes --retrieve "$data_dir" --generate "$generate" --tfds "$tfds"
