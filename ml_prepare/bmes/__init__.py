...
rand_cache_obj = Cache(fname=path.join(package_root, '_data', '.cache', 'rand_cache.pkl'))
rand_cache_recreate = environ.get('RECREATE_RAND_CACHE', all((isinstance(rand_cache_obj.fname, string_types),
...
def _populate_imgs(img_directory, skip_save=True):
    ...
    if 'id2ideyefname' not in pickled_cache or not len(pickled_cache['id2ideyefname']):
        if not path.isdir(img_directory) or not path.isdir(path.join(img_directory, 'BMES1Images')):
...
def _vanilla_stats(skip_save=True):
    ...
    if '_vanilla_stats' not in pickled_cache or not len(pickled_cache['_vanilla_stats']):
    ...
    if 'loags_id2fname' not in pickled_cache or not len(pickled_cache['loags_id2fname']):
...
def get_data(base_dir, split_dir,
             ...
             skip_save=False, cache_fname=None, max_imgs=None,
             invalidate=True):  # still saves once
    """
    ...
    if invalidate:
        cache.invalidate()
    pickled_cache = cache.update_locals(cache.load(), locals()) if path.getsize(
        path.join(package_root, 'bmes', 'generated_types.py')) > 50 else {}
    ...
    if 'glaucoma_fnames' not in pickled_cache or not len(pickled_cache['glaucoma_fnames']):
        all_imgs = pickled_cache['all_imgs']
        loags_id2fname = pickled_cache['loags_id2fname']
        roags_id2fname = pickled_cache['roags_id2fname']
