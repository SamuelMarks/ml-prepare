...
from os import environ, listdir, makedirs, mkdir, path, symlink
...
from ml_prepare.utils import create_random_numbers, ensure_is_dir, it_consumes
...
def symbolically_link(symlink_dir, df):  # type: (str, pd.DataFrame) -> pd.DataFrame
    ...
    random_list = get_or_generate_and_store_random_list(
        len(_uniq_syms),
        path.join(
            path.dirname(path.dirname(__file__)), "_data", ".cache", "dr_spoc_rand.pkl"
        ),
    )
...
def main(
    ...
    it_consumes(
        map(mkdir, filterfalse(path.isdir, (path.dirname(manual_dir), manual_dir)))
    )
